...
class StateGraph:
    ...
    def manipulable_nodes(self):
        ...
        return [x for x in list(self.graph.nodes())
    ...
    def attach_node(self, node, pre_node, edge_info):
        ...
        self.graph.nodes[node]['abs_position'] = list(
            np.array(self.graph.nodes[pre_node]['abs_position']) + np.array(p_change))
    ...
    def recomputeConfig(self):
        ...
        for l in range(len(self.layers) - 1):
            for node in self.layers[l + 1]:
                ...
                p_change = self.graph.edges[pre_node, node]['position'] + \
                           [pre_node.shape[2] + node.shape[2]]
                self.graph.nodes[node]['abs_position'] = list(
                    np.array(self.graph.nodes[pre_node]['abs_position']) +
                    np.array(p_change))

    def ConfigList(self):
        p = np.array(self.graph.node[self.root_node]['abs_position']) - \
            self.root_node.shape[2]
        Clist = {x.name: list(np.array(self.graph.nodes[x]['abs_position']) - p)
    ...
    def ifSame(self, SG):
        ...
        if EISub.number_of_nodes() == SG.graph.number_of_nodes() \
                and EISub.number_of_nodes() == self.graph.number_of_nodes():
            return True
        else:
            return False
    ...
    def recomputeLayers(self):
        self.layers = []
        last_layer = [x for x in list(self.graph.nodes())
...
class Action:
    ...
    def ifeligible(self):
        return self.check_precondition() and \
               self.check_actcondition() and \
               self.check_poscondition()
...
class MoveTo(Action):
    ...
    def check_poscondition(self):
        self.add_effect()
        return self.pos_state_graph.ifStable() and \
               not self.pos_state_graph.ifCollide()

    def add_effect(self):
        pos_SG = self.state_graph.copy()
        pos_SG.dettach_node(self.opt[0])
        pos_SG.attach_node(self.opt[0], self.opt[1],
                           {'position': self.opt[2]})
        self.pos_state_graph = pos_SG

    def show(self):
        print('Move block:', self.opt[0].name, 'to',
              self.opt[1].name, 'at position', self.opt[2])

    def configMove(self):
        return {self.opt[0].name:
                    self.pos_state_graph.graph.nodes[self.opt[0]]['abs_position']}


class MoveSubTo(Action):
    # action options: (SubassemblyToMove, ObjectToPutOn,
    # root_node postion(x,y,z,quaternions))
    # SubassemblyToMove: list of nodes, list[0] is
    #  the root node of this Subassembly
    def check_actcondition(self):
        act_SG = self.state_graph.copy()
    ...
    def check_poscondition(self):
        self.add_effect()
        return self.pos_state_graph.ifStable()\
               and not self.pos_state_graph.ifCollide()

    def add_effect(self):
        pos_SG = self.state_graph.copy()
        for node in list(self.state_graph.graph.predecessors(self.opt[0][0])):
            pos_SG.graph.remove_edge(node, self.opt[0][0])
        pos_SG.graph.add_edges_from([(self.opt[1],
                                      self.opt[0][0],
                                      {'position': self.opt[2]})])
    ...
    def show(self):
        ...
        print('Move Subassembly:', nodes_name, 'to',
              self.opt[1].name, 'at position', self.opt[2])

    def configMove(self):
        return {node.name:
                    self.pos_state_graph.graph.nodes[node]['abs_position']
                for node in self.opt[0]}
...
def ExactIdenticalSubgraph(SG1, SG2):
    ...
    for d_layer in range(min([len(SG1.layers), len(SG2.layers)])):
        layer_ide_nodes = []
        common_nodes = [x for x in SG1.layers[d_layer]
                        if x in SG2.layers[d_layer]]
        for node in common_nodes:
            ifedges = False
            if list(SG1.graph.predecessors(node)) == \
                    list(SG2.graph.predecessors(node)):
                ifedges = True
                for p in SG1.graph.predecessors(node):
                    if not SG1.graph.edges[p, node] == \
                           SG2.graph.edges[p, node]:
                        ifedges = False
...
def ConfigIdenticalSubgraph(SG1, SG2):
    ...
    for node in common_nodes:
        common_successors = [x for x in list(G1.successors(node))
                             if x in list(G2.successors(node))]
        for successor in common_successors:
            if G1.edges[node, successor] == G2.edges[node, successor]:
                CISub.add_edges_from([(node, successor,
                                       G1.edges[node, successor])])
...
def computeHeuristics(SG_cur, SG_goal):
    ...
    return len(Nodes_goal) + len(Nodes_cur) - 2 * len(Nodes_ide) \
           - len(Nodes_com) - len(Nodes_irrelevant)
...
def DFSearch(SG_start, SG_goal):
    ...
    while not NoSolutionFound and not GoalFound:  # stop condition is not satisfied:
        ...
        if not searchGraph.nodes[cur_node]['Expanded']:
